function _cobrowser_load() {
  ...
  (function ($) {
    ...
    Cobrowse = {
      debug: false,
      connected: false,
      mouse: { x: 0, y: 0, width: $(window).width(), height: $(window).height(), id: $.cookie("cobrowse_mouse_id") },
      ...
      init: function() {
        ...
        if (!Cobrowse.mouse.id) {
          if ($.cookie("cobrowse_mouse_id")) {
            Cobrowse.mouse.id = Cobrowse.mouse.id;
          } else {
        ...
        if ("faye is connected" == "faye is connected") {
          ...
          if (!$.cookie("cobrowse_dont_sent_next_page_change")) {
            if ($("#cobrowse").attr("data-remote") != "true" && !$("#cobrowser").attr("data-url") && $("#cobrowser").attr("data-url") != document.URL) {
              ...
              Cobrowse.faye.publish(Cobrowse.channel + "/event", { event: "page_change", id: Cobrowse.mouse.id, url: url, html: html.find("body").html() });
            }
          } else {
