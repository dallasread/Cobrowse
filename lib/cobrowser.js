  ...
  (function ($) {
    ...
    Cobrowse = {
      ...
      addStyle: function() {
        ...
        if ($("#cobrowser").data("style")) { Cobrowse.stylesheet = $("#cobrowser").data("style"); }
        var ss = document.createElement("link");
        ss.setAttribute("rel", "stylesheet");
        ss.setAttribute("type", "text/css");
        ss.setAttribute("href", Cobrowse.stylesheet);
        document.getElementsByTagName("head")[0].appendChild(ss);
        Cobrowse.log("Cobrowse style added.")
      },
  
      init: function() {
        if (btoa(window.location.hostname).replace("=", "") == $("#cobrowser").data("api-key")) {
          Cobrowse.addStyle();
          Cobrowse.createCta();
          Cobrowse.createControlCentre();
          Cobrowse.createArrows();
          $.cookie("cobrowse_page", document.URL);
      ...
      connect: function() {
        ...
        if ("faye is connected" == "faye is connected") {
          ...
          if (!$("#cobrowser").attr("data-url") && $("#cobrowser").attr("data-url") != document.URL) {
            ...
            Cobrowse.faye.publish(Cobrowse.channel + "/event", { event: "page_change", id: Cobrowse.mouse.id, url: url, html: html.html() });
          }
            ...
            if (Cobrowse.mouse.id != m.id) {
              if (m.event == "mousemove") {
                ...
                var js, id = 'cobrowser', ref = newDoc.getElementsByTagName('script')[0];
      					...
      					js = newDoc.createElement('script'); js.id = id; js.async = true; js.setAttribute("data-url", m.url);
      ...
      activate: function(code) {
        ...
        Cobrowse.channel = "/v1" + btoa(window.location.hostname) + "/" + Cobrowse.activation_code;
    ...
    $(document).on("click", "html, body", function(e) {
      if (Cobrowse.connected) {
        Cobrowse.faye.publish(Cobrowse.channel + "/event", { event: "click", id: Cobrowse.mouse.id, target: $(e.target).getPath() });
      }
    });
